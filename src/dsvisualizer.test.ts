...
describe('stack', () => {
  ...
  it('peek returns the top without mutating items', () => {
    ...
    expect(values(peeked.next.items)).toEqual([10, 20])
  })
...
describe('queue', () => {
  ...
  it('front returns the head without mutating items', () => {
    ...
    expect(values(front.next.items)).toEqual([5, 9])
  })
...
describe('array', () => {
  ...
  it('append grows the array', () => {
    ...
    expect(values(state.items)).toEqual([10, 20])
  })
  ...
  it('insert places a value at an index', () => {
    ...
    expect(values(inserted.next.items)).toEqual([10, 15, 20])
  })
  ...
  it('insert at length appends at the end', () => {
    ...
    expect(inserted.event.kind).toBe('ok')
    expect(values(inserted.next.items)).toEqual([1, 2])
  })
  ...
  it('remove deletes by index and returns the value', () => {
    ...
    expect(values(removed.next.items)).toEqual([20, 30])
  })
...
pop()`,
    ...
    expect(values(frames[0]?.after.items ?? [])).toEqual([3])
    expect(values(frames[1]?.after.items ?? [])).toEqual([3, 7])
    expect(frames[2]?.event.returnValue).toBe(7)
    expect(values(frames[2]?.after.items ?? [])).toEqual([3])
  })
