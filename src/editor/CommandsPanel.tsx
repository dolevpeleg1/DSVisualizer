...
export function CommandsPanel({ structureId }: CommandsPanelProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[var(--bg-panel)] text-[var(--text)]">
      ...
      <ul className="min-h-0 flex-1 space-y-3 overflow-auto p-3">
        {commands.map((command) => (
          <li key={command.signature} className="border-l-2 border-[var(--border)] pl-3">
            <code className="block font-[family-name:var(--mono)] text-sm text-[var(--text)]">
              {command.signature}
            </code>
            <p className="mt-0.5 text-xs text-[var(--text)] opacity-55">
              {command.description}
            </p>
          </li>
        ))}
      </ul>
      <p className="border-t border-[var(--border)] px-3 py-2 text-[11px] text-[var(--text)] opacity-55">
        Arguments are numbers. Use <code className="text-[var(--text)] opacity-100">//</code> for
        comments.
      </p>
    </div>
  )
